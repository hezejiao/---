	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			...
			convertView = LayoutInflater.from(mContext).inflate(
					R.layout.photo, null);
			holder.mImageView1=(ImageView) convertView.findViewById(R.id.my_photos);
		  ...
		  //从缓存中获取图片，很重要否则会导致页面闪动
      	  ...
      	  if(bitmap == null){
	      		...
	      		if(imagePath.indexOf("http://")!=-1){
					...
					Bitmap mBitmap = AbFileUtil.getBitmapFromSD(new File(imagePath), AbConstant.ORIGINALIMG, mWidth, mHeight);
					if(mBitmap!=null){
